...
subset(mydata, R == 2, select = c(Review,Payroll_Name,Age,Sub_Department))
# Review vs Age
...
ddply(mydata,.(Edu),summarise,median=median(R),mean=mean(R),count=sum(!is.na(Review)),percent=percent(sum(!is.na(Review))/nrow(mydata)),meanVS=round(mean(VS),2),meanAge=round(mean(Age),2))
...
```{r MBO, echo=FALSE, message=FALSE, warning=FALSE}
# boxplot
ggplot(mydata, aes(x=matched$Sub_Department,MBO)) + geom_boxplot() + coord_flip() + labs(x= "", y = "Review")

# Sub_Department by scores hist
ggplot(mydata, aes(x=factor(MBO),fill=matched$Sub_Department)) + geom_bar() + labs(y = "") + theme(legend.title=element_blank()) # this line revome the legend title

subset(mydata, MBO != 0.6, select = c(Review,Payroll_Name,MBO,Sub_Department))
...
subset(mydata, R == 2, select = c(Review,Payroll_Name,Children,Sub_Department))
```
...
subset(mydata, R == 2, select = c(Review,Payroll_Name,VS,Sub_Department))
