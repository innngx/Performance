...
ggplot(TE, aes(x=factor(Review), fill=Employee_Status_Type)) +  geom_bar(position="fill") + labs(x = "Review",y = "") + theme(legend.title=element_blank()) # this line revome the legend title
...
ggplot(mydata, aes(x=Age, fill=Review)) + geom_histogram(binwidth = 5) + labs(y = "") + annotate("text", label = paste ("Average Age =",round(mean(mydata$Age),1)), x = 55, y = 81, family="serif", fontface="italic", colour="darkred", size=6) + annotate("text", label = paste ("Median Age =",round(median(mydata$Age),1)), x = 55, y = 75, family="serif", fontface="italic", colour="darkred", size=6)

# Review vs Children
ggplot(mydata, aes(Children, ..count..)) + labs(y = "") + geom_density(position = "fill", aes(fill = Review, colour = Review))
ggplot(mydata, aes(x=factor(Children), fill=Review)) + geom_bar(position="fill") + labs(x = "Children",y = "")
# Children: Hist and mean label
ggplot(mydata, aes(x=Children, fill=Review)) + geom_histogram(binwidth = 1) + labs(y = "") + annotate("text", label = paste ("Average Children =",round(mean(mydata$Children),1)), x = 4.8, y = 150, family="serif", fontface="italic", colour="darkred", size=6)

# Review vs VS
mydatavs <- subset(mydata, VS != "29.25")
mydatavs <- subset(mydatavs, VS != "32.17") #to keep consistent: remove the VS 30 - Review 2 point
ggplot(mydatavs, aes(VS, ..count..)) + labs(y = "") + geom_density(position = "fill", aes(fill = Review, colour = Review)) 
# VS: Hist and Mean, median label
ggplot(mydata, aes(x=VS,fill=Review)) + geom_histogram() + labs(y = "") + annotate("text", label = paste ("Average VS =",round(mean(mydata$VS),1)), x = 30, y = 85, family="serif", fontface="italic", colour="darkred", size=6) + annotate("text", label = paste ("Median VS =",round(median(mydata$VS),1)), x = 30, y = 75, family="serif", fontface="italic", colour="darkred", size=6)

# Review vs all marital status
ggplot(mydata, aes(x=matched$Marital_Status, fill=Review)) +  geom_bar(position="fill") + labs(x = "Marital Status",y = "")
...
ggplot(mydata, aes(x=matched$Marital_Status,fill=Review)) + geom_bar() + labs(x = "Marital Status",y = "") + annotate("text", label = paste ("Marriage Rate:",percent(mean(mydata$M_Status))), x = 3, y = 330, family="serif", fontface="italic", colour="darkred", size=6)
...
ggplot(mydata, aes(matched$Gender, fill=Review)) + geom_bar() + labs(x = "Gender", y = "") + annotate("text", label = paste ("Female:",percent((1-mean(Gender)))), x = 1, y = 150, family="serif", fontface="italic", colour="darkred", size=6)
...
ggplot(TE, aes(x=Employee_Status_Type, fill=factor(Review))) +  geom_bar(position="fill") + labs(x = "Review", y = "") + scale_fill_discrete(guide = guide_legend(title = "Review")) # title text
...
ggplot(matched, aes(matched$Employee_Status_Type, fill=factor(Review))) + geom_bar() + labs(x = "", y = "") + annotate("text", label = paste ("Active:",percent((mean(Active)))), x = 1, y = 350, family="serif", fontface="italic", colour="darkred", size=5) + scale_fill_discrete(guide = guide_legend(title = "Review")) # title text
